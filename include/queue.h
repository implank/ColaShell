...
(elm)->field.le_prev=&LIST_FIRST((head));												\
while(LIST_NEXT((*(elm)->field.le_prev),field)){																		\
(elm)->field.le_prev=																					\
&LIST_NEXT((*(elm)->field.le_prev),field);									\
}																																\
LIST_NEXT((*(elm)->field.le_prev),field)=(elm);									\
}while(0)*/
...
#define LIST_INSERT_TAIL(head, elm, field) do { \
	...
	while(LIST_NEXT((listelm),field)!=NULL)listelm=LIST_NEXT((listelm),field); \
